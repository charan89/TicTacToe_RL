...
class TicTacToe():
    ...
    def step(self, curr_state, curr_action):
        ...
        obs2=self.state_transition(obs,curr_action) # state obs2= s', Agent took current action
        if self.is_terminal(obs2)[1] == 'Resume': #checking if agent action resulted in resume state for game to continue
            e_curr_action=random.choice(list(self.action_space(obs2)[1])) # choosing enviroment action
            obs3=self.state_transition(obs2,e_curr_action) # state after taking enviroment action , s''
